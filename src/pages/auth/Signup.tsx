...
const Signup: React.FC = () => {
    ...
    const loadData = async () => {
      try {
        ...
        if (rolesData.length > 0 && !formData.user_role) {
          setFormData(prev => ({ ...prev, user_role: rolesData[0].role_name }));
        }
        if (departmentsData.length > 0 && !formData.user_department) {
          setFormData(prev => ({ ...prev, user_department: departmentsData[0].department_name }));
        }
      } catch (error) {
        console.error('Error loading roles and departments:', error);
        setError('Failed to load roles and departments. Please refresh the page.');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.user_password !== (document.getElementById('confirmPassword') as HTMLInputElement)?.value) {
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-slate-800 flex-col items-center justify-center px-8">
        ...
        <div className="text-center text-white">
          <h1 className="text-2xl font-bold mb-4">Join the Employee Portal</h1>
          <p className="text-slate-300 text-lg">
            Create your account to start managing your work and collaborating with your team.
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, change_password_next_login: e.target.checked })
                  }
                  disabled={isLoading}
                />
                <label htmlFor="change_password_next_login" className="ml-2 block text-sm text-gray-700">
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, password_never_expires: e.target.checked })
                  }
                  disabled={isLoading}
                />
                <label htmlFor="password_never_expires" className="ml-2 block text-sm text-gray-700">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gray-800 text-white py-2 px-4 rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 disabled:opacity-50 disabled:cursor-not-allowed font-medium"
            >
              {isLoading ? (
